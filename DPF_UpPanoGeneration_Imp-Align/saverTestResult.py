...
class Saver(object):
    ...
    def save_samples(self, rgbs, pre_UpIMG, p_r_Pred, p_r_gt, input_up_gts, batch_idx, pred_depths, UpIMGbyPreAng):
        ...
        for i in range(rgbs.shape[0]):
            ...
            with open('H:\\DPF_UpPanoGeneration_Imp-Align\\Test_Result\\results\\PitchRollAngGT.txt', 'a') as file:
                p_r_gt_str=str(p_r_gt.tolist())
                p_r_gt_str=p_r_gt_str.replace('[','')
                p_r_gt_str=p_r_gt_str.replace(']','')
                file.write(p_r_gt_str+'\n') ## 真值
            with open('H:\\DPF_UpPanoGeneration_Imp-Align\\Test_Result\\results\\PitchRollAngPRED.txt', 'a') as file:
